...
class BetaEncoder(object):
    ...
    def transform(self, df, stat_type, N_min=1):
        ...
        return value
data = { 'salesperson': ['Alice', 'Bob', 'Alice', 'Bob', 'Alice', 'Charlie'], 
         'sales_amount': [200, 150, 300, 200, 100, 250] }

df = pd.DataFrame(data)

print(f'original df :\n{df}')

# extract category and target column
cat_cols = ['salesperson']
target = 'sales_amount'

# label encoding
df_label = df.copy()
le = LabelEncoder()
df_label['salesperson'] = le.fit_transform(df_label['salesperson'])
...
for c in cat_cols:
    ...
    feature_cols.append(feature_name)
print('\nAfter BTE :\n')
print(train[[target] + feature_cols])